...
Page({
  ...
  // 获取导航
  async getGroupList() {
    let getGroupListData = await request("/video/group/list");
    this.setData({
      groupList: getGroupListData.data.slice(0, 18),
      navId: getGroupListData.data[0].id,
    });
  ...
  handlePlay(event) {
    ...
    需求：
      ...
      2. 在播放新的视频前关闭上一个正在播放的视频
    关键：
      1. 如何找到上一个视频的实例对象
      2. 如何确认点击播放的视频和正在播放的视频不是同一个视频
    单例模式：
      1. 需要创建多个对象的场景下，通过一个变量，始终保持只要一个对象
      2. 节省内存空间
     */
    let vid = event.currentTarget.id;
    /* // 关闭上一个播放的视频 -- 当实现图片image优化video标签切换后，这里判断连续视频播放就不需要了
